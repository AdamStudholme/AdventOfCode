`use strict`
const game =`Game 1: 4 green, 3 blue, 11 red; 7 red, 5 green, 10 blue; 3 green, 8 blue, 8 red; 4 red, 12 blue; 15 red, 3 green, 10 blue
...
// if the bag contained only 12 red cubes, 13 green cubes, and 14 blue cubes ? Sum Game Ids
...
//split into array of games
const gamesArr = game.split(`\n`)
const ObjArr = []
let idCount = 0;
//split games in to draws
gamesArr.forEach((game, index) => {
    const colon = game.indexOf(`:`)
    const drawsStr = game.slice(colon + 1, game.length)
    const draws = drawsStr.split(`;`)
    let gameObj = { id: index + 1, red: 0, blue: 0, green: 0 }
    let blue = 0
    let red = 0
    let green = 0
    //Games passed in
    draws.forEach(draw => {        
        const drawArr = draw.split(`,`)
        // ObjArr.push(drawArr)
        //draws passed in
        drawArr.forEach((str ) => {
           
            const col = str.slice(1, str.length).split(` `)
            console.log(col, index);
            console.log(col[0]);
            console.log(col[1]);            
                if(col[1] === `blue` && Number(col[0]) > blue) {blue = col[0]}
                if(col[1] === `red` && Number(col[0]) > red) {red = col[0]}
                if(col[1] === `green` && Number(col[0]) > green) {green = col[0]}
            });
    })
    console.log(blue, red, green);
    //if (blue <= blueCount && red <= redCount && green <= greenCount) idCount += (index + 1)
    return idCount += (blue*red*green)
})
