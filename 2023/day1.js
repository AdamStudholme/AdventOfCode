...
const numReplace = function (str) {
  let newStr = str;
  // console.log(str.replace(/two/g, `2`));
  let indexArray = new Array(9);
  indexArray[0] = str.replace(/one/g, `1`).indexOf(`1`);
  indexArray[1] = str.replace(/two/g, `2`).indexOf(`2`);
  indexArray[2] = str.replace(/three/g, `3`).indexOf(`3`);
  indexArray[3] = str.replace(/four/g, `4`).indexOf(`4`);
  indexArray[4] = str.replace(/five/g, `5`).indexOf(`5`);
  indexArray[5] = str.replace(/six/g, `6`).indexOf(`6`);
  indexArray[6] = str.replace(/seven/g, `7`).indexOf(`7`);
  indexArray[7] = str.replace(/eight/g, `8`).indexOf(`8`);
  indexArray[8] = str.replace(/nine/g, `9`).indexOf(`9`);

  let indexArrayLast = new Array(9);
  indexArrayLast[0] = str.replace(/one/g, `1`).lastIndexOf(`1`);
  indexArrayLast[1] = str.replace(/two/g, `2`).lastIndexOf(`2`);
  indexArrayLast[2] = str.replace(/three/g, `3`).lastIndexOf(`3`);
  indexArrayLast[3] = str.replace(/four/g, `4`).lastIndexOf(`4`);
  indexArrayLast[4] = str.replace(/five/g, `5`).lastIndexOf(`5`);
  indexArrayLast[5] = str.replace(/six/g, `6`).lastIndexOf(`6`);
  indexArrayLast[6] = str.replace(/seven/g, `7`).lastIndexOf(`7`);
  indexArrayLast[7] = str.replace(/eight/g, `8`).lastIndexOf(`8`);
  indexArrayLast[8] = str.replace(/nine/g, `9`).lastIndexOf(`9`);

  const replaceMethods = (newStr, i, end) => {
    let string = "";
    const arr = [
      newStr.replace(/one/g, `1`),
      newStr.replace(/two/g, `2`),
      newStr.replace(/three/g, `3`),
      newStr.replace(/four/g, `4`),
      newStr.replace(/five/g, `5`),
      newStr.replace(/six/g, `6`),
      newStr.replace(/seven/g, `7`),
      newStr.replace(/eight/g, `8`),
      newStr.replace(/nine/g, `9`),
    ];
    string = arr[i];
    console.log(str);

    console.log(indexArrayLast);
  ...
  for (let index = 0; index < indexArray.length; index++) {
    ...
    if (min === -1 || element < indexArray[min]) min = index;
    if (max === -1 || element > indexArrayLast[max]) max = index;
  }

  const newStrMin = replaceMethods(str, min, `min`);
  const newStrMax = replaceMethods(str, max, `max`);
  console.log(newStrMin, newStrMax);

  return [newStrMin, newStrMax];
};

const firstLastDigit = (data) => {
  const strings = data.split(`\n`);
  const newStrings = [];
  strings.forEach((str) => {
    newStrings.push(numReplace(str));
  });
  let total = 0;
  let firstNum;
  let lastNum;
  ...
  newStrings.forEach((str) => {
    firstNum = /\d/.exec(str[0]);
    lastNum = /\d/.exec(str[1].split("").reverse().join(""));
    combined = Number(firstNum[0] + lastNum[0]);
    total += combined;
  });
...
const newCalc = (data) => {
  ...
  let maxInd = [];
  strings.forEach((str) => {
    minInd.push([1, str.indexOf(`1`), str.indexOf(`one`)]);
    minInd.push([2, str.indexOf(`2`), str.indexOf(`two`)]);
    minInd.push([3, str.indexOf(`3`), str.indexOf(`three`)]);
    minInd.push([4, str.indexOf(`4`), str.indexOf(`four`)]);
    minInd.push([5, str.indexOf(`5`), str.indexOf(`five`)]);
    minInd.push([6, str.indexOf(`6`), str.indexOf(`six`)]);
    minInd.push([7, str.indexOf(`7`), str.indexOf(`seven`)]);
    minInd.push([8, str.indexOf(`8`), str.indexOf(`eight`)]);
    minInd.push([9, str.indexOf(`9`), str.indexOf(`nine`)]);

    str = str.split(``).reverse().join("");
    console.log(str);

    maxInd.push([1, str.indexOf(`1`), str.indexOf(`eno`)]);
    maxInd.push([2, str.indexOf(`2`), str.indexOf(`owt`)]);
    maxInd.push([3, str.indexOf(`3`), str.indexOf(`eerht`)]);
    maxInd.push([4, str.indexOf(`4`), str.indexOf(`ruof`)]);
    maxInd.push([5, str.indexOf(`5`), str.indexOf(`evif`)]);
    maxInd.push([6, str.indexOf(`6`), str.indexOf(`xis`)]);
    maxInd.push([7, str.indexOf(`7`), str.indexOf(`neves`)]);
    maxInd.push([8, str.indexOf(`8`), str.indexOf(`thgie`)]);
    maxInd.push([9, str.indexOf(`9`), str.indexOf(`enin`)]);
  });
...
// console.log(firstLastDigit(s2Data)); //
// console.log(firstLastDigit(s2Data));

// console.log("two1nine".replace(/two/, "2"));

console.log(newCalc(s2Data));
